...
public class Bishop : Figure
{
    private void Start()
    {
        step_animation = 0.35f;

        this.name = "Bishop" + (this.transform.position.x) + " " + (this.transform.position.z); 
        ...
        for (int i = -3; i <= 3; i += 2)
        {
            if (i == -3 || i == 3) 
            {
                for (float j = 0; j < _Board.get_lenght(); j++)
                {
                    ...
                    if (cell.get_figure_in_cell()) { break; }
    ...
    public override bool check_enemy_block()
    {
        ...
        for (float i = (x_coor - 1f); i <= (x_coor + 1f); i++)
        {
            ...
            for (float j = (z_coor - 1f); j <= (z_coor + 1f); j++)
            {
                ...
                if ((Mathf.Abs(x_coor - i) == 1) && (Mathf.Abs(z_coor - j) == 1))
                {
